...
The same `minimum_fundamental_current` threshold suppresses noise-only results: below it, Current Peak and Current
Crest Factor are `unknown`. A local `peak_interval` on a main or circuit overrides the global value.

Peak measurement preserves even single-sample pulses; it does not discard the largest samples or smooth them.
An ADC or multiplexer glitch can therefore also raise Peak and Crest Factor: these values alone cannot distinguish
interference from a real short pulse. Conversely, a pulse between samples can be missed. This is a sampled peak,
not a guaranteed capture of every transient. A peak below the measured RMS current is treated as invalid.
