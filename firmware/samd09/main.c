...
volatile uint32_t DiagPowerTimingSamples = 0;
...
__attribute__((noinline))
static void update_power_timing_max_diagnostic(int32_t current_difference, int32_t latest_minus2,
...
void irq_handler_dmac(void) //We've configured it to enable Channel Transfer Complete interrupt and Channel Transfer Error interrupt.
{
	...
	for (int ct = 0; ct < 3; ct++)
	{
		...
		if (power_timing_diagnostic_enabled)
		{
			int voltage_min2 = DMAresults[lastindexMin2][ct * 2] - averages[ct];
			int voltage_min4 = DMAresults[lastindexMin4][ct * 2] - averages[ct];
			voltage_latest_minus2[ct] = voltage_differences[ct] - voltage_min2;
			voltage_latest_minus4[ct] = voltage_differences[ct] - voltage_min4;
			voltage_min2_minus4[ct] = voltage_min2 - voltage_min4;
		}
	}
	...
	for (int ct = 0; ct < 3; ct++)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (timing_diagnostic_active)
				update_power_timing_max_diagnostic(dif1, voltage_latest_minus2[i], voltage_latest_minus4[i],
	...
	for (int i = 0; i < 2; i++)
	{
		...
		{
			if (timing_diagnostic_active)
				update_power_timing_max_diagnostic(dif1, voltage_latest_minus2[x], voltage_latest_minus4[x],
